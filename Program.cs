    ...
    internal class Program
    {
...

        {
            ...
            do
            {
                ...
                else if (nameEntered.Length < minNameLength || nameEntered.Length > maxNameLength)
                {
            ...
            } while ((string.IsNullOrWhiteSpace(nameEntered)) || !(nameEntered.All(c => char.IsLetter(c) || c.Equals(' '))) || nameEntered.Length < minNameLength || nameEntered.Length > maxNameLength);
            ...
            while (!int.TryParse(ageInput, out ageEntered) || ageEntered < minAge || ageEntered > maxAge)
            {
            ...
            if (ageEntered >= minAgeLimit && ageEntered <= maxAge)
            {
            ...
            else
            {
                ...
                else if (ageConfirm.Equals("N") || ageConfirm.Equals("n"))
                {
                    ...
                    while (!int.TryParse(Console.ReadLine(), out ageEntered) || ageEntered < minAgeLimit || ageEntered > maxAge)
                    {
                        Console.WriteLine("--------------------------------\nYou are too young to play this quiz.");
                        return;
                    }
                    if (ageEntered >= minAgeLimit)
                    {
            ...
            static void easyQuiz()
            {
                ...
                Console.WriteLine("--------------------------------\nYour final score is " + userPoints + "/" + easyQuestions.Length);
            ...
            static void mediumQuiz()
            {
                ...
                for (int i = 0; i < mediumQuestions.Length; i++)
                {
                    ...
                    Console.WriteLine("--------------------------------\nYour final score is " + userPoints + "/" + mediumQuestions.Length);
                }
            }
            ...
            static void hardQuiz()
            {
                ...
                Console.WriteLine("--------------------------------\nYour final score is " + userPoints + "/" + hardQuestions.Length);
            }
        }
    }
}
